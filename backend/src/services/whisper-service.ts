...
function buildPythonTranscribeScript({ inputPath, outputDir, config }: BuildPythonTranscribeScriptOptions): string {
...
def run_openvino_transcription():
    try:
        from transformers import WhisperProcessor
    except ImportError:  # pragma: no cover - handled at runtime
        WhisperProcessor = None  # type: ignore[assignment]
    ...
    if WhisperProcessor is not None:
  ...
  return {
    async transcribe({ inputPath, outputDir, config }): Promise<WhisperTranscriptionResult> {
      ...
      if (isLikelyPythonCommand(command)) {
        ...
        const scriptContent = buildPythonTranscribeScript({ inputPath, outputDir, config });
